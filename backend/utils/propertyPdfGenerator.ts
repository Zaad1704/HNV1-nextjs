...
/**
 * Generates a comprehensive property details PDF
 * @param property The property object with populated data
 * @param stream The writable output stream to pipe the PDF to
 * @param language The language for translations
 */
export const generatePropertyDetailsPdf = (property: any, stream: Writable, language: string = 'en') => {
  ...
  const totalRent = (property.tenants || []).reduce((sum: number, tenant: any) => sum + (tenant.rentAmount || 0), 0);
  const occupiedUnits = (property.tenants || []).filter((t: any) => t.status === 'Active').length;
  ...
  if (property.tenants && property.tenants.length > 0) {
    property.tenants.forEach((tenant: any, index: number) => {
      if (yPos > 650) {
        doc.addPage();
  ...
  // Recent payments section (if available)
  if (property.recentPayments && property.recentPayments.length > 0) {
    ...
    property.recentPayments.slice(0, 3).forEach((payment: any, index: number) => {
      if (index % 2 === 0) {
...
/**
 * Generates a property financial summary PDF
 * @param property The property object with financial data
 * @param stream The writable output stream to pipe the PDF to
 * @param language The language for translations
 */
export const generatePropertyFinancialSummary = (property: any, stream: Writable, language: string = 'en') => {
  const doc = new PDFDocument({ 
    margin: 50,
    size: 'A4',
    info: {
  ...
  const totalRent = tenants.reduce((sum: number, tenant: any) => sum + (tenant.rentAmount || 0), 0);
  const totalCollected = payments.reduce((sum: number, payment: any) => sum + (payment.amount || 0), 0);
  ...
  doc.text(`Maintenance: $${expenses.filter((e: any) => e.category === 'maintenance').reduce((sum: number, e: any) => sum + e.amount, 0).toLocaleString()}`, 322, 200);
  doc.text(`Utilities: $${expenses.filter((e: any) => e.category === 'utilities').reduce((sum: number, e: any) => sum + e.amount, 0).toLocaleString()}`, 322, 220);
  doc.text(`Other: $${expenses.filter((e: any) => !['maintenance', 'utilities'].includes(e.category)).reduce((sum: number, e: any) => sum + e.amount, 0).toLocaleString()}`, 322, 240);
