...
/**
 * Generates a colorful PDF export for various data types
 * @param data The data to export
 * @param request The export request details
 * @param filePath The path to save the PDF file
 */
export const generateColorfulPdfExport = async (data: any[], request: any, filePath: string): Promise<string> => {
  ...
  // Header with gradient background based on export type
  let headerGradient;
  switch(request.type) {
    case 'properties':
  ...
  // Data section with accent color based on type
...
/**
 * Get fields to display based on export type
 */
function getExportFields(type: string): string[] {
  switch(type) {
    case 'properties':
