...
exports.deleteUnit = exports.getUnitById = exports.searchUnits = exports.getVacantUnits = exports.bulkUpdateUnitNicknames = exports.bulkUpdateUnits = exports.createUnitsForProperty = exports.updateUnitNickname = exports.updateUnit = exports.getUnits = void 0;
...
const updateUnit = async (req, res) => {
    ...
    try {
        ...
        const updatedUnit = await Unit_1.default.findByIdAndUpdate(unitId, filteredUpdates, { new: true, runValidators: true }).populate('tenantId', 'name email phone status rentAmount');
        if (!updatedUnit) {
...
const updateUnitNickname = async (req, res) => {
    const user = req.user;
    if (!user || !user.organizationId) {
        res.status(401).json({ success: false, message: 'Not authorized' });
        return;
    ...
    try {
        ...
        const unit = await Unit_1.default.findOneAndUpdate({ _id: unitId, organizationId: user.organizationId }, { nickname, alternativeName }, { new: true, runValidators: true }).populate('tenantId', 'name email status');
        if (!unit) {
            res.status(404).json({ success: false, message: 'Unit not found' });
            return;
        }
...
exports.updateUnitNickname = updateUnitNickname;
const createUnitsForProperty = async (req, res) => {
    const user = req.user;
    if (!user || !user.organizationId) {
        res.status(401).json({ success: false, message: 'Not authorized' });
        return;
    ...
    try {
        ...
        const existingUnits = await Unit_1.default.find({
            propertyId,
        ...
        const createdUnits = await Unit_1.default.insertMany(unitDocs);
        const totalUnits = await Unit_1.default.countDocuments({ propertyId });
        if (totalUnits > property.numberOfUnits) {
            await Property_1.default.findByIdAndUpdate(propertyId, { numberOfUnits: totalUnits });
        }
        res.status(201).json({
            success: true,
...
exports.createUnitsForProperty = createUnitsForProperty;
    ...
    try {
        ...
        for (const update of updates) {
            try {
                ...
                const updatedUnit = await Unit_1.default.findByIdAndUpdate(update.unitId, filteredUpdate, { new: true, runValidators: true });
                if (updatedUnit) {
                    results.push({
...
exports.bulkUpdateUnits = bulkUpdateUnits;
const bulkUpdateUnitNicknames = async (req, res) => {
    const user = req.user;
    if (!user || !user.organizationId) {
        res.status(401).json({ success: false, message: 'Not authorized' });
        return;
    ...
    try {
        ...
        const result = await Unit_1.default.bulkWrite(bulkOps);
        res.json({
            success: true,
            data: {
...
exports.bulkUpdateUnitNicknames = bulkUpdateUnitNicknames;
...
const searchUnits = async (req, res) => {
    ...
    try {
        ...
        const units = await Unit_1.default.find(filter)
...
const deleteUnit = async (req, res) => {
    ...
    try {
        ...
        if (unit.historyTracking?.totalTenants > 0) {
            await Unit_1.default.findByIdAndUpdate(unitId, { status: 'Archived' });
            res.json({
                success: true,
            ...
            await Unit_1.default.findByIdAndDelete(unitId);
            res.json({
                success: true,
