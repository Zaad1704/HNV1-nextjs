...
class MasterDataService {
    ...
    async ensureDefaultPlans() {
        try {
            const existingPlans = await Plan_1.default.countDocuments();
            if (existingPlans === 0) {
                const defaultPlans = [
                    {
                        name: 'Free Trial',
    ...
    async ensureDefaultSiteSettings() {
        try {
            const existingSettings = await SiteSettings_1.default.findOne();
            if (!existingSettings) {
                const defaultSettings = new SiteSettings_1.default({
    ...
    async getSystemStats() {
        try {
            const stats = {
                totalPlans: await Plan_1.default.countDocuments(),
                activePlans: await Plan_1.default.countDocuments({ isActive: true }),
...
exports.default = new MasterDataService();
