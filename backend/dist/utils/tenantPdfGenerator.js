...
exports.generateTenantLeaseAgreement = exports.generatePersonalDetailsPdf = exports.generateComprehensiveTenantPdf = exports.generateTenantDetailsPdf = void 0;
const pdfkit_1 = __importDefault(require("pdfkit"));
const generateTenantDetailsPdf = (tenant, stream, language = 'en') => {
    ...
    if (tenant.additionalAdults && tenant.additionalAdults.length > 0) {
        ...
        tenant.additionalAdults.forEach((adult, index) => {
            ...
            doc.fontSize(10).font('Helvetica').fillColor('#334155');
            if (adult.name)
                doc.text(`Name: ${adult.name}`, 70, adultY + 15);
            if (adult.phone)
                doc.text(`Phone: ${adult.phone}`, 70, adultY + 30);
            if (adult.relation)
                doc.text(`Relation: ${adult.relation}`, 70, adultY + 45);
            if (adult.govtIdNumber)
                doc.text(`ID: ${adult.govtIdNumber}`, 300, adultY + 15);
            if (adult.fatherName)
                doc.text(`Father: ${adult.fatherName}`, 300, adultY + 30);
            if (adult.motherName)
                doc.text(`Mother: ${adult.motherName}`, 300, adultY + 45);
    ...
    if (tenant.payments && tenant.payments.length > 0) {
        ...
        tenant.payments.slice(0, 8).forEach((payment, index) => {
            if (index % 2 === 0) {
...
exports.generateTenantDetailsPdf = generateTenantDetailsPdf;
const generateComprehensiveTenantPdf = async (tenant, stream, language = 'en') => {
    const doc = new pdfkit_1.default({
        margin: 40,
        size: 'A4',
        info: {
...
exports.generateComprehensiveTenantPdf = generateComprehensiveTenantPdf;
const generatePersonalDetailsPdf = (tenant, stream, language = 'en') => {
    try {
        const doc = new pdfkit_1.default({
            margin: 40,
            size: 'A4',
...
exports.generatePersonalDetailsPdf = generatePersonalDetailsPdf;
const generateTenantLeaseAgreement = (tenant, stream, language = 'en') => {
    const doc = new pdfkit_1.default({
        margin: 50,
        size: 'A4',
        info: {
...
exports.generateTenantLeaseAgreement = generateTenantLeaseAgreement;
