    ...
    try {
        ...
        if (this.isNew || this.isModified('status') || this.isModified('propertyId') || this.isModified('unit')) {
            ...
            if (this.propertyId && this.unit) {
                ...
                await Unit.findOneAndUpdate({ propertyId: this.propertyId, unitNumber: this.unit }, {
                    status: unitStatus,
                    tenantId: tenantId,
                ...
                if (this.isModified('unit') && !this.isNew) {
                    ...
                    if (originalUnit && originalUnit !== this.unit) {
                        await Unit.findOneAndUpdate({ propertyId: this.propertyId, unitNumber: originalUnit }, {
                            status: 'Available',
                            tenantId: null
                        });
                    }
...
TenantSchema.post('save', async function (doc) {
    try {
        ...
        const activeTenants = await this.constructor.countDocuments({
...
TenantSchema.pre('deleteOne', { document: true, query: false }, async function (next) {
    try {
        ...
        await Promise.allSettled([
            Payment.updateMany({ tenantId: this._id, status: { $ne: 'Paid' } }, { status: 'Cancelled', cancelledAt: new Date() })
        ]);
        next();
    }
    catch (error) {
...
TenantSchema.statics.findByProperty = function (propertyId, organizationId) {
    return this.find({ propertyId, organizationId }).populate('propertyId', 'name address');
};
TenantSchema.statics.findActive = function (organizationId) {
    return this.find({ organizationId, status: 'Active' }).populate('propertyId', 'name');
};
TenantSchema.statics.findExpiring = function (organizationId, days = 30) {
...
TenantSchema.statics.findLate = function (organizationId) {
